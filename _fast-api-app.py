...
from signing import Document, Signature, SIGNATURES, SIGNED
...
async def sign_document(request: Request, filename: str = None, signature: str = None):
    ...
    signature = Signature(os.path.join(SIGNATURES, signature))
    
    res = file_to_sign.add_signature(signature, (330, 90))
    return templates.TemplateResponse("sign.html", {"request": request, "filename": filename, "signature": signature, "res": res})
